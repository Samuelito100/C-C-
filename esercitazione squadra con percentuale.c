...
int main(){
int i,infortunati,giocatori,max_rand,non_infortunato,num;
float percentuale,num_percentuale;
...
for ( i = 0; i < giocatori; i++)
{
    printf("giocatore %d\n",i);
    num=rand()%max_rand;
    num_percentuale=num/1000;
    if (num_percentuale<=percentuale)
    {
...
if (infortunati==0)
    {
        printf("facciamo la riunione giovedi, abbiamo %d infortunati\n",infortunati);
